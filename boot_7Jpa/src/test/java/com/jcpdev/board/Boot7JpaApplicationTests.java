...
class Boot7JpaApplicationTests {
	...
	public void insertBoard() {
		for(int i=1;i<20;i++) {
			...
			// random.nextInt(9) 메소드는 0 <= 랜덤 < 9 , Long.valueOf : int를 Long 타입변환
			Users user = Users.builder()
					.mno(Long.valueOf(random.nextInt(4)+3)).build();
			Board board=Board.builder().subject("제목"+i)
	...
	//users 테이블의 save,find,delete
	public void delete() {
		udao.deleteById(2L);   //delete 기본키값 조건 쿼리에 해당합니다. 
	}
	public void getOne() {
		...
		//orElse()메소드는 users 가 null 일때 Users 객체를 생성하여 반환합니다.
		//  			null 이 아니면 users 가 참조하는 Users 객체를 반환합니다.
	...
	void insertUser() {
		Users user = Users.builder()
				...
				.name("모모")
				.build();
		udao.save(user);		//save 메소드는 insert,update sql 에 해당됩니다.
	}
